...
$bhCmd = "Write-Host 'BH Log' -ForegroundColor Cyan; Get-Content '$bhLog' -Wait -Tail 50"
$wbCmd = "Write-Host 'WB Log' -ForegroundColor Green; Get-Content '$wbLog' -Wait -Tail 50"
wt -w 0 new-tab --title "BH Log" pwsh -NoExit -Command $bhCmd `; split-pane --title "WB Log" pwsh -NoExit -Command $wbCmd
...
Write-Host '  Copy-Item -Recurse .\dev\sample-orders\pickup-bh S:\HitePhotoTest\BH\dakis-watch'
